...
main() {
  ...
  custom_path="$(get_tmux_option "@onedark_custom_plugin_dir" "${PLUGIN_DIR}/custom")"
  ...
  # shellcheck source=./onedark-frappe.tmuxtheme
  while IFS='=' read -r key val; do
    ...
    color_values+=("${temp}")
  done <"${PLUGIN_DIR}/themes/onedark_${theme}.tmuxtheme"

  # status general
  local status_default status_justify status_background message_background
  status_default=$(get_tmux_option "@onedark_status_default" "on")
  ...
  status_background=$(get_tmux_option "@onedark_status_background" "theme")
  ...
  pane_status_enable=$(get_tmux_option "@onedark_pane_status_enabled" "no") # yes
  pane_border_status=$(get_tmux_option "@onedark_pane_border_status" "off") # bottom
  pane_border_style=$(
    get_interpolated_tmux_option "@onedark_pane_border_style" "fg=${thm_gray}"
  )
  pane_active_border_style=$(
    get_interpolated_tmux_option "@onedark_pane_active_border_style" \
      "#{?pane_in_mode,fg=${thm_yellow},#{?pane_synchronized,fg=${thm_magenta},fg=${thm_orange}}}"
  )
  pane_left_separator=$(get_tmux_option "@onedark_pane_left_separator" "█")
  pane_middle_separator=$(get_tmux_option "@onedark_pane_middle_separator" "█")
  pane_right_separator=$(get_tmux_option "@onedark_pane_right_separator" "█")
  pane_number_position=$(get_tmux_option "@onedark_pane_number_position" "left") # right, left
  ...
  window_status_separator=$(get_interpolated_tmux_option "@onedark_window_separator" "")
  setw window-status-separator "$window_status_separator"

  window_left_separator=$(get_tmux_option "@onedark_window_left_separator" "█")
  window_right_separator=$(get_tmux_option "@onedark_window_right_separator" "█")
  window_middle_separator=$(get_tmux_option "@onedark_window_middle_separator" "█ ")
  window_number_position=$(get_tmux_option "@onedark_window_number_position" "left") # right, left
  window_status_enable=$(get_tmux_option "@onedark_window_status_enable" "no")       # right, left
  ...
  status_left_separator=$(get_tmux_option "@onedark_status_left_separator" "")
  status_right_separator=$(get_tmux_option "@onedark_status_right_separator" "█")
  status_connect_separator=$(get_tmux_option "@onedark_status_connect_separator" "yes")
  status_fill=$(get_tmux_option "@onedark_status_fill" "icon")

  status_modules_left=$(get_tmux_option "@onedark_status_modules_left" "")
  ...
  status_modules_right=$(get_tmux_option "@onedark_status_modules_right" "application session")
